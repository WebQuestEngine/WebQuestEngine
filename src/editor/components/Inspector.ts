...
export class Inspector {
                  ...
                  <div style="display:grid; grid-template-columns:1fr 1fr; gap:6px; margin-top:4px;">
                    ...
                    <div>
                      ...
                      <select class="form-select act-anim-target" data-hidx="${hIdx}" data-aidx="${aIdx}" data-ischar="${isCharacter ? 'true' : 'false'}" style="font-size:0.75rem;">
                        ...
                        ${(this.currentScene?.characters || []).map(c => `
  ...
  private attachEvents(): void {
    ...
    this.element.querySelectorAll('.act-type-select').forEach(select => {
      select.addEventListener('change', (e) => {
        const targetEl = e.target as HTMLSelectElement;
        const hIdx = parseInt(targetEl.dataset.hidx!);
        const aIdx = parseInt(targetEl.dataset.aidx!);
    ...
    this.element.querySelectorAll('.act-event-name').forEach(input => {
      input.addEventListener('input', (e) => {
        const targetEl = e.target as HTMLInputElement;
        const hIdx = parseInt(targetEl.dataset.hidx!);
        const aIdx = parseInt(targetEl.dataset.aidx!);
    ...
    this.element.querySelectorAll('.act-event-payload').forEach(input => {
      input.addEventListener('input', (e) => {
        const targetEl = e.target as HTMLInputElement;
        const hIdx = parseInt(targetEl.dataset.hidx!);
        const aIdx = parseInt(targetEl.dataset.aidx!);
    ...
    this.element.querySelectorAll('.act-anim-target').forEach(select => {
      select.addEventListener('change', (e) => {
        const targetEl = e.target as HTMLSelectElement;
        const hIdx = parseInt(targetEl.dataset.hidx!);
        const aIdx = parseInt(targetEl.dataset.aidx!);
    ...
    this.element.querySelectorAll('.act-target-spawn-x').forEach(input => {
      input.addEventListener('input', (e) => {
        const targetEl = e.target as HTMLInputElement;
        const hIdx = parseInt(targetEl.dataset.hidx!);
        const aIdx = parseInt(targetEl.dataset.aidx!);
    ...
    this.element.querySelectorAll('.act-target-spawn-y').forEach(input => {
      input.addEventListener('input', (e) => {
        const targetEl = e.target as HTMLInputElement;
        const hIdx = parseInt(targetEl.dataset.hidx!);
        const aIdx = parseInt(targetEl.dataset.aidx!);
    ...
    this.element.querySelectorAll('.act-clear-flag').forEach(input => {
      input.addEventListener('input', (e) => {
        const targetEl = e.target as HTMLInputElement;
        const hIdx = parseInt(targetEl.dataset.hidx!);
        const aIdx = parseInt(targetEl.dataset.aidx!);
  ...
  private syncCharacterAcrossScenes(targetChar: CharacterData): void {
    VisualSpritePickerModal.syncCharacterAcrossScenes(this.project, targetChar);
  }
}
